...
import {DEFAULT_CONFIG,DEFAULT_DISPLAY,GAME_MODES,DIFFICULTIES,matchPlan} from '../../game/config.mjs';
import {COCS_TIERS,DEFAULT_COCS_TIER,DIRECTOR_TIERS,directorTierCopy} from '../../game/cocs-difficulty.mjs';
...
import {modeTargetText,objectiveCopy} from '../../game/hud.mjs';
...
export function AccessibilityConfiguration({accessibility,onChange,display,onDisplayChange}:any){
 const value=normalizeAccessibility(accessibility);
 const set=(patch:any)=>onChange(normalizeAccessibility({...value,...patch}));
 // The display-backed readouts (captions, reduce motion, UI text scale) moved
 // here from the Video panel so every accessibility answer lives in one place.
 // The page keeps normalizing the merged patch, so this stays a pure view.
 const view=display&&typeof display==='object'?display:null;
 const patchDisplay=(patch:any)=>onDisplayChange?.({...view,...patch});
...
export function PresetsConfiguration({presets=[],onSave,onLoad,onDelete}:any){const [name,setName]=useState('');const full=presets.length>=PRESET_LIMIT;return <div className="config-block presets-configuration"><h3>Loadout presets</h3><div className="preset-save"><input aria-label="Preset name" placeholder="Preset name" maxLength={24} value={name} onChange={e=>setName(e.target.value)}/><button className="secondary-button" onClick={()=>{onSave?.(name);setName('');}}>SAVE CURRENT</button></div><p className={`config-note preset-capacity${full?' is-full':''}`}>{presets.length} / {PRESET_LIMIT} SAVED{full&&presets[0]?` · SAVING REPLACES “${presets[0].name}”`:''}</p>{presets.length?<div className="preset-list">{presets.map((p:any)=><div key={p.id} className="preset-row" style={{flexWrap:'wrap'}}><span style={{minWidth:0}}>{p.name}<small>{p.character} / {p.harness}{p.mapId?` · ${p.mapId}`:''}</small></span><div className="preset-actions"><button className="text-button" aria-label={`Load preset ${p.name}`} onClick={()=>onLoad?.(p)}>LOAD</button><button className="text-button" aria-label={`Delete preset ${p.name}`} onClick={()=>onDelete?.(p.id)}>DELETE</button></div></div>)}</div>:<p className="config-note">Save your operator, harness, arena and rules for one-tap recall.</p>}</div>;}

export function MatchConfiguration({config,onChange,excludeModes=[]}:any){const patch=(key:string,value:any)=>onChange({...config,[key]:value});const mode=GAME_MODES.find(m=>m.id===config.mode)!;const rules:any=mode?.rules||{};const target=scoreRule(mode);
 // F06: one launch view derived from normalizeConfig + mode metadata. Every
 // promise on this screen (duration, roster, auto-fill, modifiers, tier) and
 // the selection cards read the same matchPlan, so the setup text cannot drift
 // from the config a match actually starts with.
 const plan=matchPlan(config);
 const coop=config.mode==='cocs-coop';
 const tierId=(plan.tier?.id??DEFAULT_COCS_TIER) as keyof typeof DIRECTOR_TIERS;
 const tier=DIRECTOR_TIERS[tierId];
 const tierCopy=directorTierCopy(tierId);
 const waveCount=plan.waves?.count??0;
 const waveCopy=plan.waves?.copy??'Director waves are enabled.';
 const setTier=(id:string)=>patch('objective',{...(config.objective||{}),tier:id});
 const selectMode=(id:string)=>{const next:any=GAME_MODES.find(m=>m.id===id);onChange({...config,mode:id,...(id==='cocs'||id==='cocs-coop'?latticePracticeDefaults(id):{}),...(id==='puma-race'&&config.mode!==id?{botCount:7}:{}),...(id==='puma-soccer'&&config.mode!==id?{botCount:3}:{}),...(next?.rules?.fragLimit!==undefined?{fragLimit:next.rules.fragLimit}: {})});};return <section id="match-setup" className="match-configuration">
 ...
 {config.mode==='puma-race'?<div className="config-grid"><div className="config-block"><h3>Race rules</h3><Range label="Rival drivers" value={config.botCount} min={0} max={7} onChange={(v:number)=>patch('botCount',v)}/><Range label="Lap limit" value={config.fragLimit} min={1} max={10} onChange={(v:number)=>patch('fragLimit',v)}/><Range label="Time limit" value={config.timeLimit/60} min={1} max={15} suffix=" min" onChange={(v:number)=>patch('timeLimit',v*60)}/><label className="config-field">Your callsign<input aria-label="Your callsign" maxLength={20} value={config.playerName} onChange={e=>patch('playerName',e.target.value)}/></label></div><div className="config-block"><h3>Equal Puma chassis</h3><p>{objectiveCopy('laps')}</p><p>WASD throttle / steer. Space or crouch handbrake. Shift chassis boost. Fire or power uses your held item; interact resets to your checkpoint.</p><p>Choose 0 to 7 AI rival drivers; zero means no AI opponents. In online races, human drivers replace excess bots to keep the grid capped at 8 racers. Combat modifiers, weapons and harness powers are disabled.</p></div></div>:config.mode==='puma-soccer'?<div className="config-grid"><div className="config-block"><h3>Soccer rules</h3><p className="config-static">2 v 2 &mdash; four Pumas total. Empty seats are filled by bots.</p><Range label="Goal limit" value={config.fragLimit} min={1} max={15} onChange={(v:number)=>patch('fragLimit',v)}/><Range label="Time limit" value={config.timeLimit/60} min={1} max={15} suffix=" min" onChange={(v:number)=>patch('timeLimit',v*60)}/><label className="config-field">Your callsign<input aria-label="Your callsign" maxLength={20} value={config.playerName} onChange={e=>patch('playerName',e.target.value)}/></label></div><div className="config-block"><h3>Equal Puma chassis</h3><p>{objectiveCopy('goals')||'Smash the ball into the enemy goal. First team to the goal target wins; the clock decides a draw.'}</p><p>WASD throttle / steer. Space or crouch handbrake. Shift chassis boost. Interact resets to your kickoff spot. Boards ring the pitch so the ball stays in play.</p><p>Bots fill the empty seats, so it is always two against two. Combat modifiers, weapons and harness powers are disabled.</p></div></div>:<div className="config-grid">
  {coop?<div className="config-block"><h3>Operations squad &amp; waves</h3><Range label="Bot seats" value={config.botCount} min={0} max={maxBotsFor(config.mode)} onChange={(v:number)=>patch('botCount',v)}/><p className="config-static"><b>{plan.rosterLabel}</b> · {waveCopy}</p><p><b>Allied fill.</b> {plan.fill.note}</p><p><b>Enemy waves.</b> The Director commits each wave&rsquo;s composition and front count from its budget; higher tiers only add fronts, events and shorter clocks, never enemy health, armour or damage.</p>{config.botCount===0&&<p><b>0 bot seats.</b> You deploy alone: no AI squad and no garrison bot seats, but all {waveCount} Director waves still spawn. Operations is never a solo sandbox.</p>}<label className="config-field">Your callsign<input aria-label="Your callsign" maxLength={20} placeholder="Use operator name" value={config.playerName} onChange={e=>patch('playerName',e.target.value)}/></label></div>
  :<div className="config-block"><h3>Opponents</h3><Range label="Bot count" value={config.botCount} min={0} max={maxBotsFor(config.mode)} onChange={(v:number)=>patch('botCount',v)}/><Choice label="Bot difficulty" value={config.difficulty} options={DIFFICULTIES.map(d=>[d.id,d.name])} onChange={(v:string)=>patch('difficulty',v)} disabled={config.botCount===0}/><p>{config.botCount===0?(plan.fill.note??'Solo practice: explore and test weapons until the timer ends.'):DIFFICULTIES.find(d=>d.id===config.difficulty)?.description} Bots use their operator's health, armor and speed, with the same match modifiers as players. Difficulty changes AI aim and reaction only.</p><label className="config-field">Your callsign<input aria-label="Your callsign" maxLength={20} placeholder="Use operator name" value={config.playerName} onChange={e=>patch('playerName',e.target.value)}/></label></div>}
  {coop?<><div className="config-block"><h3>Director tier</h3><Choice label="Director tier" value={tierId} options={COCS_TIERS.map(id=>[id,`${id} · ${DIRECTOR_TIERS[id as keyof typeof DIRECTOR_TIERS].label}`])} onChange={setTier}/><p className="config-static">{tierId} · {tier.label} — {tierCopy.copy}</p><div className="row">{tierCopy.modifiers.map(m=><span className="chip" key={m}>{m}</span>)}</div><p>Wave pressure only: fronts, reinforcement events, denial behaviour and the wave clock. Director tiers never change AI health, armour, damage or speed.</p><Choice label="AI difficulty · aim & reaction" value={config.difficulty} options={DIFFICULTIES.map(d=>[d.id,d.name])} onChange={(v:string)=>patch('difficulty',v)}/><p>{DIFFICULTIES.find(d=>d.id===config.difficulty)?.description} Separate from the Director tier: this changes how AI operators aim and react, not which waves attack.</p></div><div className="config-block"><h3>Operation rules</h3><Range label="Time limit" value={config.timeLimit/60} min={1} max={15} suffix=" min" onChange={(v:number)=>patch('timeLimit',v*60)}/><Range label="Respawn delay" value={config.respawn} min={1} max={5} suffix=" sec" onChange={(v:number)=>patch('respawn',v)}/><p>{mode.description}</p></div></>
...
export function DisplayConfiguration({display,onChange}:any){const patch=(key:string,value:any)=>onChange({...display,[key]:value});return <div className="config-block display-configuration"><h3>View & crosshair</h3>
 ...
 <Choice label="Resolution cap" value={display.resolutionCap??'auto'} options={[['auto','Auto'],['1080p','1080p'],['1440p','1440p'],['native','Native']]} onChange={(v:string)=>patch('resolutionCap',v)}/><Choice label="Detail level" value={display.quality??'auto'} options={[['auto','Auto'],['low','Low'],['medium','Medium'],['high','High']]} onChange={(v:string)=>patch('quality',v)}/><Choice label="Frame rate cap" value={display.fpsCap??0} options={[[0,'Uncapped'],[30,'30 FPS'],[60,'60 FPS'],[120,'120 FPS']]} onChange={(v:string)=>patch('fpsCap',Number(v))}/><Choice label="Shadows" value={display.shadows??'high'} options={[['high','High'],['low','Low'],['off','Off']]} onChange={(v:string)=>patch('shadows',v)}/><Range label="Field of view" value={display.fov} min={65} max={110} suffix="°" onChange={(v:number)=>patch('fov',v)}/><Choice label="Crosshair shape" value={display.crosshair} options={['cross','dot','ring','chevron','split'].map(v=>[v,v[0].toUpperCase()+v.slice(1)])} onChange={(v:string)=>patch('crosshair',v)}/><p className="config-note">Team colours and high contrast moved to the Accessibility panel below.</p><Range label="Crosshair size" value={display.size} min={.6} max={1.8} step={.1} suffix="×" onChange={(v:number)=>patch('size',v)}/><div className="crosshair-advanced" role="group" aria-label="Crosshair detail">
  <Range label="Crosshair gap" value={display.crosshairGap??0} min={0} max={4} step={.5} suffix="px" onChange={(v:number)=>patch('crosshairGap',v)}/>
