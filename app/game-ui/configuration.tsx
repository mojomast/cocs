...
export function DisplayConfiguration({display,onChange}:any){const patch=(key:string,value:any)=>onChange({...display,[key]:value});return <div className="config-block display-configuration"><h3>View & crosshair</h3>
  ...
  <Range label="UI text scale" value={Math.round((display.uiScale??1)*100)} min={80} max={140} step={10} suffix="%" onChange={(v:number)=>patch('uiScale',v/100)}/><p className="config-note">Scales the command board, spend window and terminal text independently of the 3D image (0.8×–1.4×).</p>
