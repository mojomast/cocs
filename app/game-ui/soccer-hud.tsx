...
export function SoccerHud({soccer,state,actorId=0,touchControls=false,controls=''}:{soccer?:any;state?:any;actorId?:number;touchControls?:boolean;controls?:string}){
 ...
 return <div className="race-hud soccer-hud" aria-label="Live soccer status"><div className="race-metrics hud-strip"><div className="hud-cell"><small>RED</small><strong>{Number(scores[0])||0}</strong></div><div className="hud-cell"><small>BLUE</small><strong>{Number(scores[1])||0}</strong></div><div className="hud-cell"><small>MATCH TIME</small><strong>{clock}</strong></div><div className="hud-cell race-coins"><small>GOAL LIMIT</small><strong>{goalLimit}</strong></div></div>{countdown&&<div className="race-countdown hud-count" role="status" aria-live="polite">{countdown}</div>}<div className="race-item hud-panel"><small>YOUR SIDE</small><strong>{teamLabel(team)}</strong><span>{goals} GOAL{goals===1?'':'S'} · {phaseLabel(phase)}</span></div><p className="race-help">{touchControls?'Left stick throttle / steer. Push fully for boost.':controls}<br/>TAB standings / ESC pause</p></div>;
}
