...
export default function Home(){
 ...
 try{const prefs=JSON.parse(localStorage.getItem('token-arena-settings')||'{}');if(typeof prefs.sensitivity==='number'){r.lookSensitivity=Math.max(.3,Math.min(2.5,Number.isFinite(prefs.sensitivity)?prefs.sensitivity:1));setSensitivity(r.lookSensitivity);}if(typeof prefs.muted==='boolean'){audio.setMuted(prefs.muted);setMuted(prefs.muted);}if(typeof prefs.musicVolume==='number'){audio.setVolume('music',prefs.musicVolume);setMusicVolume(Math.max(0,Math.min(1,prefs.musicVolume)));}if(typeof prefs.effectsVolume==='number'){audio.setVolume('effects',prefs.effectsVolume);setEffectsVolume(Math.max(0,Math.min(1,prefs.effectsVolume)));}if(typeof prefs.ambienceVolume==='number'){audio.setVolume('ambience',prefs.ambienceVolume);setAmbienceVolume(Math.max(0,Math.min(1,prefs.ambienceVolume)));}if(typeof prefs.masterVolume==='number')audio.setVolume('master',prefs.masterVolume);if(typeof prefs.showcase==='boolean'){r.showcaseEnabled=prefs.showcase;setShowcase(prefs.showcase);}if(typeof prefs.legacyArenas==='boolean'){r.legacyArenas=prefs.legacyArenas;setLegacyMaps(prefs.legacyArenas);}if(typeof prefs.voiceVolume==='number'&&Number.isFinite(prefs.voiceVolume)){const v=Math.max(0,Math.min(1,prefs.voiceVolume));voicePrefs.current.volume=v;setVoiceVolume(v);}if(typeof prefs.voiceThreshold==='number'&&Number.isFinite(prefs.voiceThreshold)){const v=Math.max(0,Math.min(1,prefs.voiceThreshold));voicePrefs.current.threshold=v;setVoiceThreshold(v);}if(typeof prefs.music==='boolean'){audio.setMusicEnabled(prefs.music);setDemoMusic(prefs.music);}if(typeof prefs.ambience==='boolean'){audio.setAmbient(prefs.ambience);setDemoAmbience(prefs.ambience);}if(typeof prefs.announcer==='boolean'){audio.setAnnouncer(prefs.announcer);setDemoAnnouncer(prefs.announcer);}if(prefs.weather===null||WEATHER_KINDS.includes(prefs.weather)){const weather=prefs.weather===null?null:prefs.weather;setDemoWeather(weather);view.setWeather(weather);}}catch{}
  ...
  const s=r.net.renderState(performance.now());
   ...
   raf=requestAnimationFrame(loop);return;
   ...
   else if(modeRef.current==='playing'&&r.match){const lessonBreak=r.training&&(r.training.phase==='complete'||r.training.done);r.acc=lessonBreak?0:Math.min(r.acc+elapsed,RULES.dt*5);while(r.acc>=RULES.dt){const p=r.match.actors[0];const localInput:any=controlsFromState({keys,touch:r.touch,look:p,move:r.touch?{x:r.touch.moveX||0,y:r.touch.moveY||0}:null,sprint:r.touch?.sprint===true,crouch:r.touch?.crouch===true,mobility:r.touch?.mobility===true,fire:r.fire,fireTap:r.fireTap,jump:r.jump,power:r.power,interact:r.interact,weapon:r.inputWeapon,ads:r.ads,reload:r.reload,melee:r.melee,grenade:r.grenade,bindings:r.bindings});const cocsOrders=r.cocsOrders?.length?r.cocsOrders.splice(0,r.cocsOrders.length):null;const cocsSpends=r.cocsSpends?.length?r.cocsSpends.splice(0,r.cocsSpends.length):null;r.match.step(RULES.dt,(cocsOrders||cocsSpends)?{...localInput,cocs:{orders:cocsOrders??[],spends:cocsSpends??[]}}:localInput);r.fireTap=false;r.jump=false;r.power=false;r.interact=false;r.inputWeapon=-1;r.reload=false;r.melee=false;r.grenade=false;r.acc-=RULES.dt;r.view.capturePresentation(r.match);if(r.match.over){
    ...
    if(r.match===r.trainingMatch){r.recorder=null;r.training=null;r.trainingEvents=[];setReward(null);setNotice('PRACTICE MATCH ENDED · Replay Field Training from Quick Start. Training does not award XP or update match records.');changeMode('selection');document.exitPointerLock?.();break;}
...
pauseRender:(on:boolean)=>{const previous=r.benchmarking===true;r.benchmarking=on===true;return previous;},spendWindow:(open:boolean,seconds=90)=>{const coop=r?.match?.objectiveState?.coop;if(!coop)return false;const dt=RULES.dt||1/60;if(open){coop.phase='intermission';coop.intermissionOpen=true;coop.intermissionTicks=Math.max(60,Math.round((Number(seconds)||90)/dt));}else{coop.intermissionOpen=false;coop.intermissionTicks=0;}return true;}};
 ...
 useEffect(()=>{if(runtime.current)runtime.current.display=display;runtime.current?.view.setDisplay(display);reducedOverride=display.reducedMotion===true;runtime.current?.view.director?.setReduced?.(reducedMotion());runtime.current?.audio?.setMothEnabled?.(!reducedMotion());},[display,ready]);
  ...
  requestLockRef.current=requestLock;
   ...
   const queueRanked=()=>{const r=runtime.current,n=r?.net;if(!r||!n?.connected)return;setNetError('');const name=config.playerName||selected.name;if(!n.players.length)n.join(name,character,harness,{roomId:'local'});n.send({type:'queue',ranked:true,name,playerId:n.playerId,progressToken:n.progressToken});queuedRef.current=true;setNetQueued({status:'queued'});};
  ...
  const quickStart=(activity:string,overrides?:{botCount?:number;difficulty?:string})=>{const r=runtime.current;if(!r)return;const training=activity==='field-training'?'cocs':activity==='operations-training'?'cocs-coop':null;if(training){const pick='lattice-slice',rules=trainingConfig(training,{playerName:config.playerName});setConfig(rules);setMapId(pick);start({character,harness,mapId:pick,config:rules,training});r.trainingMatch=r.match;return;}if(activity==='spectate'){startSpectate();return;}if(activity==='horde'||activity==='campaign'){setSingleSub(activity);startSinglePlayer(activity);return;}const allowed=mapsForMode(activity,{legacy:legacyMaps}),pick=allowed.some((m:any)=>m.id===mapId)?mapId:(allowed[0]?.id??mapId),rules=normalizeConfig({...config,mode:activity,...latticePracticeDefaults(activity),...(Number.isFinite(overrides?.botCount)?{botCount:overrides!.botCount}:{}),...(overrides?.difficulty?{difficulty:overrides.difficulty}:{})});setConfig(rules);setMapId(pick);start({character,harness,mapId:pick,config:rules});};
  const startTraining=(mode:string)=>quickStart(mode==='cocs-coop'?'operations-training':'field-training');
  const continueTutorial=()=>{const r=runtime.current;if(!r?.training)return;r.training=continueTraining(r.training,{time:r.match?.time??r.training.lastTime});r.trainingEvents=[];setHud((previous:any)=>previous?{...previous,training:trainingView(r.training)}:previous);};
  const endTutorial=()=>{const r=runtime.current;if(!r?.training)return;r.training=skipTraining(r.training);r.training=null;r.trainingEvents=[];setHud((previous:any)=>previous?{...previous,training:null}:previous);};
 ...
 const resume=()=>{runtime.current?.audio.start();changeMode('playing');requestLock();};
  ...
  const player=hud?.actors?.find((a:any)=>a.id===(hud.actorId??0))||hud?.actors?.[0],activePower=HARNESSES.find((h:any)=>h.id===player?.harness),hudMode=GAME_MODES.find((m:any)=>m.id===hud?.config?.mode),brief=commandBrief(hud,player,hudMode),hudMap=getMap(hud?.mapId||mapId),hudRoute=routeContext(hudMap,player),phase=matchPhase(hud);
   ...
   const issueCocsOrder=()=>{const r=runtime.current,state=cocsNow(),snapshot=hud?.cocs,team=cocsTeam(),tick=Number(snapshot?.tick)||0,cardId=`cocs-${team}-${tick}-${Number(state.seq)||0}`,peerId=latticePeerId(r?.net,r?.match);
    ...
    if(gate&&gate.ok!==true){applyCocsStrip({...result.state,pending:null,issued:null,notice:`REJECTED · ${String(gate.reason).toUpperCase()}`});return;}
    applyCocsStrip(result.state);if(!r)return;
   ...
   const activateCocsBoardCard=(card:any,action:string)=>{
    ...
    if(card.source===LOCAL_CARD_SOURCE.ORDER||action===LOCAL_CARD_ACTION.ISSUE){
     ...
     if(gate&&gate.ok!==true){notify(false,`${verb} REJECTED · ${String(gate.reason).toUpperCase()}`);return {ok:false,reason:String(gate.reason)};}
     ...
     notify(true,`${verb} ${card.targetLabel} · ORDER ACCEPTED`);return {ok:true,reason:null};
    }
   ...
   const spendCocs=(verb:string,target:any)=>{
    ...
    r.cocsSpendSeq=seq;const cardId=`spend-${team}-${tick}-${seq}`;
...
const cocsCommand=cocsView?{...cocsView,boardView:mergedBoard??cocsView.boardView,spend:cocsSpendView??cocsView.spend,armCocsVerb,pickCocsTarget,issueCocsOrder,cancelCocsStrip,spendCocs,spendVisible,skipSpend,reopenSpend,notice:cocsNotice,onReadoutPanel:toggleReadoutPanel,cursorKey:keyLabel(bindings.cursor??DEFAULT_BINDINGS.cursor),selectBoardCard:selectCocsBoardCard,activateBoardCard:activateCocsBoardCard,closeBoard:closeCocsBoard,toggleBoardPin:toggleCocsBoardPin,boardOpen:cocsBoard.open===true&&!boardCollapsed,boardCollapsed,boardPinned:cocsBoard.pinned===true,boardActive:cocsBoardIds[cocsBoard.active??0]??cocsBoardIds[0]??null}:null;
   const aimActor=player||hud?.actors?.[0],aimWeapon=aimActor?WEAPONS[aimActor.weapon??0]||WEAPONS[0]:null,aimSpread=aimActor?effectiveSpread(aimActor,aimWeapon,{handling:harnessWeaponHandling(aimActor.harness,aimActor.weapon)}):0,crosshairGap=dynamicCrosshairGap(aimSpread,display.size),reloadFill=reloadProgress(aimActor),reloading=Boolean(aimActor?.reloading),posture=postureLabel(aimActor),marker=hitMarker(hud,player),ammoEmpty=Boolean(player&&typeof player.ammo?.[player.weapon]==='number'&&player.ammo[player.weapon]===0),ammoLow=lowAmmo(player,WEAPONS);
    ...
    useEffect(()=>{
     ...
     const r=runtime.current;if(!r)return;
     r.caption='INTERMISSION · SPEND WINDOW OPEN · SPEND FLUX OR SKIP';
     r.captionAt=Number(hud?.time)||0;
