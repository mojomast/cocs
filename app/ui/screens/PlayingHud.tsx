...
export function PlayingHud({ui}:ScreenProps){
 ...
 const streak=!hud.spectate?streakStatus(player):null;
  ...
  {hud.spectate?<div className="spectator-tag" role="status" aria-live="polite"><span className="eyebrow">YOU ARE SPECTATING</span><h2>NO SEAT</h2><p>Following {player.name} live. Snapshot feed only — no inputs sent.</p><p className="spectator-controls">[ / ] follow · H hide HUD · P third person · ESC lobby</p></div>:player.health>0?<><div className={`crosshair shape-${display.crosshair}`} style={{'--crosshair-color':display.color,'--crosshair-size':display.size,'--crosshair-gap':`${crosshairGap}px`} as any}><span/><span/><span/><span/></div>{marker&&<div className={`hitmarker ${marker}`} role="status" aria-live="polite" aria-label={marker==='kill'?'Elimination confirmed':'Hit confirmed'}><span/><span/></div>}{reloading&&<div className="reload-indicator" role="status" aria-live="polite" aria-label={`Reloading ${Math.round(reloadFill*100)} percent`}><span className="reload-label">RELOADING</span><span className="reload-track"><i style={{width:`${Math.round(reloadFill*100)}%`}}/></span></div>}{posture&&<div className="posture-chip" role="status">{posture}</div>}</>:<div className="death-message"><span className="eyebrow">CONNECTION LOST</span><h2>RECOMPILING</h2><p>Respawning in {Math.max(1,Math.ceil(player.dead))}…</p></div>}
