...
export function PlayingHud({ui}:ScreenProps){
 ...
 const streak=!hud.spectate?streakStatus(player):null;
  ...
  {hud.spectate?null:player.health>0?<><SightReticle ui={ui}/>{marker&&<div className={`hitmarker ${marker}`} aria-hidden="true"><span/><span/></div>}{reloading&&<div className="reload-indicator" aria-hidden="true"><span className="reload-label">RELOADING</span><span className="reload-track"><i style={{width:`${Math.round(reloadFill*100)}%`}}/></span></div>}{posture&&<div className="posture-chip">{posture}</div>}</>:<div className="death-message"><span className="eyebrow">CONNECTION LOST</span><h2>RECOMPILING</h2><p>Respawning in {Math.max(1,Math.ceil(player.dead))}…</p></div>}
