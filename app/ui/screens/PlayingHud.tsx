 ...
 const streak=!hud.spectate?streakStatus(player):null;
  ...
  <div className="hud-bottom" role="region" aria-label="Player status">
   ...
   <div className="hud-corner hud-corner--right">
    ...
    {!hud.spectate&&!vehicle&&movement&&<div className={`ability-card movement-card${movement.ready?' is-ready':''}${movement.phase!=='ready'?' is-active':''}${movement.enabled?'':' is-disabled'}`} role="group" aria-label={`Movement ${movement.name} · ${movement.note}`}>
    ...
    {frag&&<div className={`ability-card frag-card${frag.ready?' is-ready':''}`} role="group" aria-label={frag.label}>
  ...
  <div className={`hud-bottom-note${hud.time>60?' is-settled':''}`}><span className="hint-static">TAB / SCOREBOARD</span>{!hud.spectate&&<span className="hint-static"><kbd>{cursor?.key??'ALT'}</kbd> / FREE CURSOR</span>}{hud.spectate&&<span className="hint-static">[ / ] FOLLOW</span>}{hud.net&&hud.quality&&<span className={`net-quality ${hud.quality.tone}`}>{hud.quality.label} · {hud.quality.ms}MS</span>}{voiceHint(voiceState.enabled,voiceState.mode,ui.bindings)&&<span>{voiceHint(voiceState.enabled,voiceState.mode,ui.bindings)}</span>}{display.showFps&&<span>{hud.fps||0} FPS · {hud.renderer==='software'?'CPU':'WEBGL'}</span>}<span className="hint-static">{escapeHint(hud.net)}</span>{reducedMotion()&&<span className="motion-note">REDUCED MOTION</span>}</div></div>;
}
