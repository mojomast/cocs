...
// ---------------------------------------------------------------------------
// VIP extraction escort (v8.6). Escorts take deterministic slots around the
// live VIP (or the beacon once the package stands on it) so the escort bubble
// that keeps the VIP advancing never thins out; hunters converge on the package
// while it lives, or on the beacon once it is down. Pure: reads the objective
// state, teams and actor id only, so replays evaluate identical orders.
export function extractionBotOrders(match,a){
 const state=match?.objectiveState;if(!state||state.kind!=='extraction')return null;
 const extract=state.extract||{x:match?.center?.x??0,y:0,z:match?.center?.z??0};
 const escortTeam=state.escortTeam??0,team=Number.isFinite(a?.team)?a.team:escortTeam;
 const vip=state.vipId==null?null:(match.actors||[]).find(actor=>actor.id===state.vipId&&actor.health>0)||null;
 const escortRadius=Number.isFinite(state.escortRadius)?state.escortRadius:7;
 const vipAtExtract=Boolean(vip)&&Math.hypot(vip.x-extract.x,vip.z-extract.z)<=escortRadius;
 if(team===escortTeam){
  // The slot ring is 60% of the escort radius, so every escort stays inside the
  // bubble that lets `stepExtractionVip` move the package.
  const anchor=vip&&!vipAtExtract?vip:extract;
  return {state:'objective',role:'escort',destination:match.zoneSlot(a,{id:'vip',x:anchor.x,z:anchor.z,y:anchor.y??0,radius:Math.max(2,escortRadius*.6),owner:escortTeam},team)};
 }
 return {state:'objective',role:'hunt',destination:vip?{x:vip.x,y:vip.y??0,z:vip.z}:{x:extract.x,y:extract.y??0,z:extract.z}};
}
