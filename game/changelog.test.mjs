...
test('the changelog is ordered newest-first with complete entries', () => {
  ...
  for (const release of releases) {
    assert.match(release.version, /^v\d+\.\d+$/, 'version is vX.Y');
  ...
  for (let i = 1; i < numbers.length; i++) {
    const [a1, a2] = numbers[i - 1], [b1, b2] = numbers[i];
    assert.ok(a1 > b1 || (a1 === b1 && a2 >= b2), `release order at index ${i} is descending`);
  }
});
