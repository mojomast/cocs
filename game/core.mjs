...
export class Match{
 ...
 rayWorld(o,d,max){return rayWorld(o,d,max,this.arena);}
    ...
    this.difficulty=DIFFICULTIES.find(d=>d.id===this.config.difficulty);this.arena=getMap(mapId);const nav=vehicleMode?{nodes:[],edges:[]}:matchNavigation(this.arena);this.nav=nav.nodes;this.edges=nav.edges;{const arenaBounds=boundsOf(this.arena);this.center={x:(arenaBounds.minX+arenaBounds.maxX)/2,z:(arenaBounds.minZ+arenaBounds.maxZ)/2};}this.spawns=this.arena.spawns.map(([x,z])=>v(x,floorAt(x,z,this.arena),z));this.random=random;this.time=0;this.over=false;this.suddenDeath=false;this.armsraceWinner=null;this.events=[];this.feed=[];this.rockets=[];this.deployables=[];this.stats={shots:0,kills:0,pickups:0,powers:0,respawns:0,falls:0};this.serial=0;this.teamScores={0:0,1:0};this.vehicleHits=new Map();
 ...
 this.pickups=this.pickups.filter(p=>{
  ...
  useful(a,p){if(this.mutators.instagib&&p.kind!=='health'&&p.kind!=='armor')return false;if(p.kind==='health')return a.health<a.maxHealth;if(p.kind==='armor')return a.armor<100;if(p.kind==='megahealth')return a.health<a.maxHealth||a.armor<100;if(p.kind==='ammo'){const n=pickupWeapon(p.weapon??a.weapon);return n!==undefined&&loadoutAllows(this.loadout,n)&&a.ammo[n]<this.weaponForIndex(a,n).cap;}if(POWERUPS.some(x=>x.id===p.kind))return true;if(economyPickup(p.kind))return true;const n=pickupWeapon(p.kind);return n!==undefined&&loadoutAllows(this.loadout,n)&&a.ammo[n]<this.weaponForIndex(a,n).cap;}
   ...
   deploySentry(a,duration){if(!a||a.health<=0)return false;const sentry={id:++this.serial,owner:a.id,team:Number.isFinite(a.team)?a.team:null,x:a.x,y:a.y??0,z:a.z,life:duration,cooldown:0,range:22,damage:9,interval:.5,health:80};this.deployables.push(sentry);this.emit('deployable',{actor:a.id,sentry:sentry.id,x:sentry.x,y:sentry.y,z:sentry.z,duration});return true;}
