...
import {passiveBonus,passiveEffect,passiveScale} from './spec-effects.mjs';
...
export function moveActor(a,input,dt,arena=MAPS[0],config={speed:1,gravity:1},ropeLines=null){
 ...
 const speed=baseSpeed*config.speed*(a.speedMultiplier||1)*(activeBuff(a,'speed')??1)*(a.slow>0?(a.slowMultiplier??.55):1)*(a.gearSpeed||1)*(a.carryingFlag?(a.carrySpeedMultiplier??.9):1);
 ...
 this.pickups=this.pickups.filter(p=>{
  ...
  _stepThreatPing(a,dt){
   ...
   for(const enemy of this.actors){
    ...
    this.emit('threat-ping',{actor:a.id,source:enemy.id,pos:target,duration:passive.duration??.75});
    return true;
   }
   return false;
  }
  ...
  if(!finitePoint(chosen)||floorAt(chosen.x,chosen.z,this.arena)===null||obstructed(chosen.x,chosen.y,chosen.z,RULES.radius,this.arena)){const bx=Number.isFinite(chosen.x)?chosen.x:this.center.x,bz=Number.isFinite(chosen.z)?chosen.z:this.center.z;let spot=null;for(let ring=1;ring<=10&&!spot;ring++)for(let i=0;i<8&&!spot;i++){const ang=i/8*Math.PI*2,x=bx+Math.cos(ang)*ring*1.25,z=bz+Math.sin(ang)*ring*1.25,y=floorAt(x,z,this.arena);if(y!==null&&!obstructed(x,y,z,RULES.radius,this.arena))spot=v(x,y,z);}chosen=spot||v(this.center.x,floorAt(this.center.x,this.center.z,this.arena)??0,this.center.z);}
    ...
    a.movementLanded=false;a.glideSteer=0;a.inputJump=false;a.inputCrouch=false;a.inputMobility=false;a.firingThisTick=false;a.threatPing=0;a.braceTimer=0;a.braceMitigation=0;a.braceKnockbackScale=1;
  ...
  if(ability.kind==='slow'){const range=(ability.radius??ability.range??h.range)*passiveScale(a.harness,'radius'),slow=ability.slow??h.magnitude;for(const b of this.actors)if(b!==a&&b.health>0&&!b.protection&&(!teamMode(this.config)||b.team!==a.team)&&dist(a,b)<range&&this.visible(eye(a),eye(b))){b.slow=ability.duration??h.duration;b.slowMultiplier=slow;this.emit('jam',{actor:b.id,pos:eye(b)});}}
   ...
   switchWeapon(a,index,{source='request'}={}){
    ...
    const holster=REVISION.swapSeconds(a.verbState,{from:a.weapon,to:index,base:.45});a.weapon=index;a.weaponSwitch=ADAPTIVE.swapDelay(a.verbState,holster);
