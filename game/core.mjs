...
export class Match{
 ...
 rayWorld(o,d,max){return rayWorld(o,d,max,this.arena);}
    ...
    this.config=normalizeConfig(options);this.mutators=mutatorEffects(this.config);this.loadout=resolveMatchLoadout(this.config);this.humanCount=Math.max(1,Math.min(Math.round(options.humanCount??1),8));
 ...
 this.pickups=this.pickups.filter(p=>{
  ...
  this.actors=[this.actor(0,character,harness),...Array.from({length:this.humanCount-1},(_,i)=>this.actor(i+1,CHARACTERS[(CHARACTERS.findIndex(c=>c.id===character)+i+1)%CHARACTERS.length].id,HARNESSES[Math.floor(this.random()*HARNESSES.length)].id)),...Array.from({length:this.config.botCount},(_,i)=>this.actor(this.humanCount+i,CHARACTERS[(CHARACTERS.findIndex(c=>c.id===character)+this.humanCount+i)%CHARACTERS.length].id,HARNESSES[Math.floor(this.random()*HARNESSES.length)].id))];this.actors[0].name=this.config.playerName||this.actors[0].name;
    ...
    actor(id,character,harness){const l=resolveLoadout(character,harness),actor={id,...l,team:teamMode(this.config)?id%2:undefined,name:CHARACTERS.find(c=>c.id===l.character).name,frags:0,deaths:0,streak:0,ladder:0,scoreStats:{captures:0,flagPickups:0,flagReturns:0,flagDrops:0,objectiveTime:0,objectiveCaptures:0,objectiveNeutralizations:0,objectiveContests:0},x:0,y:0,z:0,lastValid:null,vx:0,vy:0,vz:0,yaw:0,pitch:0,bodyYaw:0,grounded:true,coyote:0,jumpBuffer:0,health:0,armor:0,dead:0,vehicleId:null,vehicleSeat:null,vehicleSeatIndex:0,weapon:this.startingLoadout().weapon,ammo:this.startingLoadout().ammo,cooldown:0,active:0,slow:0,slowMultiplier:.55,shotWait:0,grenadeCooldown:0,protection:0,shots:0,traversalCooldown:0,traversalPad:null,traversalTarget:null,zipRide:null,carryingFlag:false,carrySpeedMultiplier:1,spread:0,punchYaw:0,punchPitch:0,punchVelYaw:0,punchVelPitch:0,reloading:false,reloadTimer:0,reloadDuration:0,reloadWeapon:-1,melee:0,weaponSwitch:0,burst:0,burstTimer:0,sprinting:false,crouching:false,sliding:false,slideTimer:0,slideCooldown:0,eyeHeight:MOVE.eyeStanding,baseHeight:MOVE.baseHeight,ads:false,jumpHeld:false,jumpCutArmed:false,powerups:{},speedMultiplier:1,damageMultiplier:1,cooldownMultiplier:1,temporaryShield:0,activeSpeedMultiplier:1,hitScale:this.mutators.bigHead?1.5:1,harnessSpeedMultiplier:1,harnessResistance:0,juggernaut:false,juggernautShield:0,juggernautDamage:1,bot:id<this.humanCount?null:{route:[],think:0,target:-1,memory:0,reaction:0,stuck:0,last:v(),state:'roam',patrol:0,flank:null,flankDone:false,recover:0,suppressed:0,threat:-1,standoff:null,strafeReverse:-99}};applyHarnessProfile(actor);actor.botScan=botScanRange(this.difficulty,this.arena);return actor;}
  ...
  if(!finitePoint(chosen)||floorAt(chosen.x,chosen.z,this.arena)===null||obstructed(chosen.x,chosen.y,chosen.z,RULES.radius,this.arena)){const bx=Number.isFinite(chosen.x)?chosen.x:this.center.x,bz=Number.isFinite(chosen.z)?chosen.z:this.center.z;let spot=null;for(let ring=1;ring<=10&&!spot;ring++)for(let i=0;i<8&&!spot;i++){const ang=i/8*Math.PI*2,x=bx+Math.cos(ang)*ring*1.25,z=bz+Math.sin(ang)*ring*1.25,y=floorAt(x,z,this.arena);if(y!==null&&!obstructed(x,y,z,RULES.radius,this.arena))spot=v(x,y,z);}chosen=spot||v(this.center.x,floorAt(this.center.x,this.center.z,this.arena)??0,this.center.z);}
    const stats=CHARACTERS.find(c=>c.id===a.character).stats,gear=a.gear||{},npc=a.npcProfile||null,maxHealth=npc?.health??stats.health+(gear.health||0),spawnArmor=Math.max(0,npc?.armor??stats.armor+(gear.armor||0)),spawn=this.startingLoadout();
      Object.assign(a,{...chosen,lastValid:{...chosen},vx:0,vy:0,vz:0,health:maxHealth,maxHealth,armor:spawnArmor,moveSpeed:npc?.moveSpeed??stats.speed,gearSpeed:(gear.speed??1)*(npc?.speedMult??1),gearDamage:(gear.damage??1)*(npc?.damageMult??1),gearSpread:gear.spread??1,dead:0,streak:0,weapon:spawn.weapon,ammo:spawn.ammo,cooldown:0,active:0,slow:0,slowMultiplier:.55,shotWait:.25,grenadeCooldown:0,protection:RULES.protection,grounded:true,jumpBuffer:0,coyote:0,spread:0,punchYaw:0,punchPitch:0,punchVelYaw:0,punchVelPitch:0,reloading:false,reloadTimer:0,reloadDuration:0,reloadWeapon:-1,melee:0,weaponSwitch:0,burst:0,burstTimer:0,burstLeft:0,sprinting:false,crouching:false,sliding:false,slideTimer:0,slideCooldown:0,eyeHeight:MOVE.eyeStanding,baseHeight:MOVE.baseHeight,ads:false,jumpHeld:false,jumpCutArmed:false,traversalFlight:false,traversalTarget:null,zipRide:null,carryingFlag:false,carrySpeedMultiplier:1,traversalCooldown:0,traversalPad:null,traversalEvent:null,yaw:Math.atan2(chosen.x,chosen.z),pitch:0,powerups:{},speedMultiplier:1,damageMultiplier:1,cooldownMultiplier:1,temporaryShield:0,activeSpeedMultiplier:1,hitScale:this.mutators.bigHead?1.5:1});applyHarnessProfile(a);a.bodyYaw=a.yaw;a.juggernaut=false;a.juggernautShield=0;a.juggernautDamage=1;if(this.objectiveState?.kind==='juggernaut'&&this.objectiveState.juggernautId===a.id){const jr=modeRule(this.config.mode);a.juggernaut=true;a.juggernautShield=jr.juggernautShield??100;a.juggernautDamage=jr.juggernautDamage??1.5;}if(this.mutators.randomLoadout&&modeWeapon(this.config,this.loadout)===null){const pool=WEAPONS.map((_,index)=>index).filter(index=>loadoutAllows(this.loadout,index));const pick=pool.length?pool[Math.floor(this.random()*pool.length)]:0;a.weapon=pick;const w=WEAPONS[pick];if(w)a.ammo[pick]=this.config.unlimitedAmmo?Infinity:Math.max(a.ammo[pick]||0,w.ammo);}if(this.config.mode==='armsrace'){const rung=Math.max(0,Math.min(WEAPONS.length-1,a.ladder??0));a.weapon=rung;const rw=WEAPONS[rung];if(rw&&!this.config.unlimitedAmmo)a.ammo[rung]=Math.max(a.ammo[rung]||0,rw.ammo);}
    ...
    damage(target,amount,source){if(this.race||this.over||target.health<=0||target.protection>0)return 0;const guardrail=target.active>0&&target.harness==='claudecode'?.5:0,mitigation=Math.min(.5,Math.max(target.harnessResistance||0,guardrail));let damage=((this.config.mode==='instagib'||this.mutators.oneShot)?10000:amount*this.mutators.damageMultiplier)*(source?.damageMultiplier||1)*(1-mitigation)*(this.mutators.berserk&&(source?.streak||0)>=3?1.2:1);const facingshield=target.npcShield;if(facingshield&&source&&source!==target){const fdx=source.x-target.x,fdz=source.z-target.z,fdist=Math.hypot(fdx,fdz);if(fdist>1e-4){const facingX=-Math.sin(target.yaw||0),facingZ=-Math.cos(target.yaw||0),dot=(fdx/fdist)*facingX+(fdz/fdist)*facingZ,arc=Number.isFinite(facingshield.arc)?facingshield.arc:.6;if(dot>=Math.cos(arc))damage*=1-(facingshield.reduction??.7);else if(dot<=-Math.cos(arc))damage*=facingshield.flankBonus??1.4;}}const shield=Math.min(target.temporaryShield||0,damage);target.temporaryShield-=shield;damage-=shield;const jugGuard=Math.min(target.juggernautShield||0,damage);target.juggernautShield=(target.juggernautShield||0)-jugGuard;damage-=jugGuard;const absorb=Math.min(Math.max(0,target.armor),damage*.6);target.armor-=absorb;damage-=absorb;const healthBefore=target.health,actual=Math.min(target.health,damage);target.health=Math.max(0,target.health-damage);
  ...
  if(this.mutators.lifeSteal&&source&&source!==target&&source.health>0)source.health=Math.min(source.maxHealth,source.health+actual*.25);this.emit('damage',{actor:target.id,source:source?.id,amount:shield+jugGuard+absorb+actual,shield:shield});
    ...
    power(a){if(this.race||this.over||a.health<=0||a.cooldown>0||this.mutators.instagib||this.flagCarrier(a))return false;const h=HARNESSES.find(h=>h.id===a.harness),ability=harnessAbility(a.harness)||h;a.protection=0;a.cooldown=(ability.cooldown??h.cooldown)*(this.mutators.fastPowers?.5:1)*a.cooldownMultiplier;a.active=ability.duration??h.duration;a.activeSpeedMultiplier=ability.speed??h.magnitude??1;this.stats.powers++;this.emit('power',{actor:a.id,harness:a.harness,pos:eye(a),duration:a.active});
  ...
  if(a.harness==='roo'){const range=ability.radius??ability.range??h.range,slow=ability.slow??h.magnitude;for(const b of this.actors)if(b!==a&&b.health>0&&!b.protection&&(!teamMode(this.config)||b.team!==a.team)&&dist(a,b)<range&&this.visible(eye(a),eye(b))){b.slow=ability.duration??h.duration;b.slowMultiplier=slow;this.emit('jam',{actor:b.id,pos:eye(b)});}}
   ...
   chainFrom(target,w,affinityDamage,source){let chained=0;for(const b of this.actors){if(chained>=w.chain)break;if(b===target||b===source||b.health<=0)continue;if(teamMode(this.config)&&source&&b.team===source.team)continue;if(dist(target,b)<=w.chainRange&&this.visible(eye(target),eye(b))){this.damage(b,w.damage*affinityDamage*.5,source);chained++;}}}
    ...
    if(this.race)return false;
     ...
     const baseWeapon=this.weaponFor(a),weapon=a.weapon,handling=harnessWeaponHandling(a.harness,weapon)||{},noRecoil=this.mutators.noRecoil,w=noRecoil?{...baseWeapon,recoil:{kick:0,recover:12,pattern:[[0,0]]},bloom:{base:0,perShot:0,max:0,recovery:.1,moveFactor:0}}:baseWeapon;let chargeScale=1;
  ...
  useful(a,p){if(this.mutators.instagib&&p.kind!=='health'&&p.kind!=='armor')return false;if(p.kind==='health')return a.health<a.maxHealth;if(p.kind==='armor')return a.armor<100;if(p.kind==='megahealth')return a.health<a.maxHealth||a.armor<100;if(p.kind==='ammo'){const n=pickupWeapon(p.weapon??a.weapon);return n!==undefined&&loadoutAllows(this.loadout,n)&&a.ammo[n]<this.weaponForIndex(a,n).cap;}if(POWERUPS.some(x=>x.id===p.kind))return true;const n=pickupWeapon(p.kind);return n!==undefined&&loadoutAllows(this.loadout,n)&&a.ammo[n]<this.weaponForIndex(a,n).cap;}
   collect(a,p){if(!this.useful(a,p))return false;const power=POWERUPS.find(x=>x.id===p.kind);if(p.kind==='health')a.health=Math.min(a.maxHealth,a.health+35);else if(p.kind==='armor')a.armor=Math.min(100,a.armor+40);else if(p.kind==='megahealth'){a.health=Math.max(a.health,Math.min(a.maxHealth,150));a.armor=Math.min(150,a.armor+75);}else if(p.kind==='ammo'){const n=pickupWeapon(p.weapon??a.weapon),w=this.weaponForIndex(a,n);a.ammo[n]=this.config.unlimitedAmmo?Infinity:Math.min(w.cap,a.ammo[n]+w.ammo);if(a.weapon===0&&this.config.mode!=='armsrace')a.weapon=n;}else if(power){a.powerups[power.id]=power.duration;this.refreshPowerups(a);if(power.effect.armor)a.temporaryShield=power.effect.armor;this.emit('powerup',{actor:a.id,kind:power.id,duration:a.powerups[power.id],effect:{...power.effect},pos:eye(a)});}else{const n=pickupWeapon(p.kind),w=this.weaponForIndex(a,n);a.ammo[n]=this.config.unlimitedAmmo?Infinity:Math.min(w.cap,a.ammo[n]+w.ammo);if(a.weapon===0&&this.config.mode!=='armsrace')a.weapon=n;}p.wait=p.kind==='health'||p.kind==='armor'?12:15;this.stats.pickups++;this.emit('pickup',{actor:a.id,kind:p.kind,powerup:!!power});return true;}
