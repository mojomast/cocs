...
export class Match{
 ...
 rayWorld(o,d,max){return rayWorld(o,d,max,this.arena);}
    ...
    if(vehicleMode){this.config.botCount=Math.min(this.config.botCount,8-this.humanCount);if(!getMap(mapId).race)mapId=this.config.mode==='puma-soccer'?'puma-pitch':'puma-circuit';}
 ...
 this.pickups=this.pickups.filter(p=>this.config.mode!=='instagib'&&(modeWeapon(this.config)===null||p.kind==='health'||p.kind==='armor'));
  ...
  leaders(){if(this.race){if(this.race.kind==='soccer'){const id=soccerStandings(this.race)[0]?.actorId;return this.actors.filter(a=>a.id===id);}const id=this.race.winnerId??raceStandings(this.race)[0]?.actorId;return this.actors.filter(a=>a.id===id);}if(teamMode(this.config)){const max=Math.max(...Object.values(this.teamScores));return this.actors.filter(a=>this.teamScores[a.team]===max);}if(this.config.mode==='armsrace'&&this.armsraceWinner!==null&&this.armsraceWinner!==undefined)return this.actors.filter(a=>a.id===this.armsraceWinner);return rankLeaders(this.actors,this.config.mode);}
