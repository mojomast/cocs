...
import {abilityOf,harnessAbility,harnessPassive,harnessVehicle,harnessWeaponHandling} from './harness-profiles.mjs';
...
export function moveActor(a,input,dt,arena=MAPS[0],config={speed:1,gravity:1}){
 ...
 const speed=(a.moveSpeed??CHARACTERS.find(c=>c.id===a.character)?.stats.speed??RULES.speed)*config.speed*(a.harnessSpeedMultiplier||1)*(a.speedMultiplier||1)*(activeBuff(a,'speed')??1)*(a.slow>0?(a.slowMultiplier??.55):1)*(a.gearSpeed||1)*(a.carryingFlag?(a.carrySpeedMultiplier??.9):1);
...
export class Match{
 ...
 this.pickups=this.pickups.filter(p=>{
  ...
  if(this.mutators.lifeSteal&&source&&source!==target&&source.health>0)source.health=Math.min(source.maxHealth,source.health+actual*.25);this.emit('damage',{actor:target.id,source:source?.id,amount:shield+jugGuard+absorb+actual,shield:shield,shieldBreak});
   ...
   if(ability.kind==='heal')a.health=Math.min(a.maxHealth,a.health+(ability.heal??h.magnitude));
  ...
  if(ability.kind==='slow'){const range=ability.radius??ability.range??h.range,slow=ability.slow??h.magnitude;for(const b of this.actors)if(b!==a&&b.health>0&&!b.protection&&(!teamMode(this.config)||b.team!==a.team)&&dist(a,b)<range&&this.visible(eye(a),eye(b))){b.slow=ability.duration??h.duration;b.slowMultiplier=slow;this.emit('jam',{actor:b.id,pos:eye(b)});}}
   if(ability.kind==='burst'){const range=ability.radius??h.range,damage=ability.damage??h.damage,knockback=ability.knockback??h.magnitude,lift=ability.lift??4;for(const b of this.actors){const d=dist(a,b);if(b!==a&&b.health>0&&(!teamMode(this.config)||b.team!==a.team)&&d<range&&this.visible(eye(a),eye(b))){this.damage(b,damage,a);const dir=norm(v(b.x-a.x,0,b.z-a.z));b.vx+=dir.x*knockback;b.vz+=dir.z*knockback;b.vy+=lift;}}}return true;}
   ...
   switchWeapon(a,index,{source='request'}={}){
    ...
    a.weapon=index;a.weaponSwitch=.45;a.reloading=false;a.reloadTimer=0;a.reloadDuration=0;a.reloadWeapon=-1;a.reloadCap=undefined;a.reloadAmount=undefined;
   ...
   chainFrom(target,w,affinityDamage,source){let chained=0;for(const b of this.actors){if(chained>=w.chain)break;if(b===target||b===source||b.health<=0)continue;if(teamMode(this.config)&&source&&b.team===source.team)continue;if(dist(target,b)<=w.chainRange&&this.visible(eye(target),eye(b))){this.damage(b,w.damage*affinityDamage*.5,source);chained++;}}}
    ...
    if(this.race)return false;
     ...
     const affinityDamage=(handling.favored?handling.damage:1)*(a.gearDamage||1)*(a.harnessDamageMultiplier||1)*chargeScale,activeFireRate=a.active>0?(1/(activeBuff(a,'fireRate')||1)):1;
     ...
     const spread=effectiveSpread(a,w,{handling,speedFrac});
