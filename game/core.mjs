...
import {createOperatorVerbState,resetOperatorVerbState,setOperatorVerbActive,stepOperatorVerbState,operatorVerbSnapshot,operatorVerbFor,ADAPTIVE,REVISION,HEAT,DEEP_COMPUTE,BRACED,ALIGNMENT_REVIEW,LONG_CONTEXT,TOOL_USE,EFFORTLESS} from './operator-verbs.mjs';
...
// window (movement-only; neutral for every other class, §3.2).
 ...
 const effortlessSlide=EFFORTLESS.slide(a.verbState),effortlessHop=EFFORTLESS.hopWindow(a.verbState),slideBoost=MOVE.slideBoost*effortlessSlide.boostMultiplier,slideFriction=MOVE.slideFriction*effortlessSlide.frictionMultiplier;
 if(crouching&&a.grounded&&!a.sliding&&(a.slideCooldown||0)<=0&&(input.sprint===true||a.sprinting)&&horizontal>6){
  a.sliding=true;a.slideTimer=MOVE.slideMin+effortlessSlide.minSecondsBonus;
 ...
 if(a.grounded){
  ...
  if(ix||iz){const before=Math.hypot(a.vx,a.vz),projection=a.vx*ix+a.vz*iz,airControl=EFFORTLESS.airControl(a.verbState),airAccel=MOVE.airAccel*airControl.airAccelMultiplier,airCap=a.glideSteer>0?a.glideSteer:MOVE.airCap*airControl.airCapMultiplier,add=Math.min(wishSpeed,airCap)-projection;
...
export class Match{
 ...
 this.pickups=this.pickups.filter(p=>{
  ...
  if(!finitePoint(chosen)||floorAt(chosen.x,chosen.z,this.arena)===null||obstructed(chosen.x,chosen.y,chosen.z,RULES.radius,this.arena)){const bx=Number.isFinite(chosen.x)?chosen.x:this.center.x,bz=Number.isFinite(chosen.z)?chosen.z:this.center.z;let spot=null;for(let ring=1;ring<=10&&!spot;ring++)for(let i=0;i<8&&!spot;i++){const ang=i/8*Math.PI*2,x=bx+Math.cos(ang)*ring*1.25,z=bz+Math.sin(ang)*ring*1.25,y=floorAt(x,z,this.arena);if(y!==null&&!obstructed(x,y,z,RULES.radius,this.arena))spot=v(x,y,z);}chosen=spot||v(this.center.x,floorAt(this.center.x,this.center.z,this.arena)??0,this.center.z);}
    ...
    a.movementLanded=false;a.glideSteer=0;a.inputJump=false;a.inputCrouch=false;a.inputMobility=false;a.firingThisTick=false;a.braceTimer=0;a.braceMitigation=0;a.braceKnockbackScale=1;
  ...
  if(ability.kind==='slow'){const range=ability.radius??ability.range??h.range,slow=ability.slow??h.magnitude;for(const b of this.actors)if(b!==a&&b.health>0&&!b.protection&&(!teamMode(this.config)||b.team!==a.team)&&dist(a,b)<range&&this.visible(eye(a),eye(b))){b.slow=ability.duration??h.duration;b.slowMultiplier=slow;this.emit('jam',{actor:b.id,pos:eye(b)});}}
   ...
   switchWeapon(a,index,{source='request'}={}){
    ...
    const holster=REVISION.swapSeconds(a.verbState,{from:a.weapon,to:index,base:.45});a.weapon=index;a.weaponSwitch=ADAPTIVE.swapDelay(a.verbState,holster);a.reloading=false;a.reloadTimer=0;a.reloadDuration=0;a.reloadWeapon=-1;a.reloadCap=undefined;a.reloadAmount=undefined;
   ...
   chainFrom(target,w,affinityDamage,source){let chained=0;for(const b of this.actors){if(chained>=w.chain)break;if(b===target||b===source||b.health<=0)continue;if(teamMode(this.config)&&source&&b.team===source.team)continue;if(dist(target,b)<=w.chainRange&&this.visible(eye(target),eye(b))){this.damage(b,DEEP_COMPUTE.clampShot(source?.verbState,w.damage*affinityDamage*.5,{targetHealth:b.maxHealth}),source);chained++;}}}
    ...
    if(this.race)return false;
     ...
     const classHandling=ADAPTIVE.handling(a.verbState),toolHandling=TOOL_USE.handling(a.verbState),intervalScale=(handling.interval??1)*classHandling.interval*toolHandling.interval,spreadHandling={...handling,interval:intervalScale,spread:(handling.spread??1)*classHandling.spread*toolHandling.spread};
     ...
     const affinityDamage=(handling.favored?handling.damage:1)*(a.gearDamage||1)*(a.harnessDamageMultiplier||1),classChargeScale=DEEP_COMPUTE.multiplier(a.verbState,{attachmentCharge:chargeScale}),rangeScale=LONG_CONTEXT.rangeMultiplier(a.verbState),activeFireRate=a.active>0?(1/(activeBuff(a,'fireRate')||1)):1;
  ...
  melee(a){if(this.over||a.health<=0||(a.melee||0)>0)return false;a.protection=0;a.melee=MELEE.cooldown;const reach=TOOL_USE.meleeRange(a.verbState,MELEE.range),dir=aim(a.yaw,a.pitch),origin=eye(a);let best=null,bestD=Infinity;for(const b of this.actors){if(b===a||b.health<=0||(teamMode(this.config)&&b.team===a.team))continue;const d=dist(a,b);if(d>reach||d>=bestD)continue;const target=eye(b),to=norm(v(target.x-origin.x,target.y-origin.y,target.z-origin.z));if((to.x*dir.x+to.y*dir.y+to.z*dir.z)<MELEE.arc)continue;if(!this.visible(origin,target))continue;best=b;bestD=d;}if(best)this.damage(best,(a.meleeDamage??MELEE.damage),a);this.emit('melee',{actor:a.id,hit:best?.id??null,pos:origin});return true;}
     explode(r,hit){const source=this.actors[r.owner],w=WEAPONS[r.weapon??1],affinityDamage=r.damageMultiplier||1,blast={x:r.pos.x,y:(r.pos.y??0)+.35,z:r.pos.z};if(hit)this.damage(hit,DEEP_COMPUTE.clampShot(source?.verbState,w.damage*affinityDamage,{targetHealth:hit.maxHealth}),source);for(const a of this.actors){const p=eye(a),d=dist(r.pos,p);if(a.health>0&&(a===source||!teamMode(this.config)||a.team!==source?.team)&&d<w.radius&&(d<w.radius*.6||this.visible(blast,p))){this.damage(a,DEEP_COMPUTE.clampShot(source?.verbState,w.splash*affinityDamage*(1-d/w.radius),{targetHealth:a.maxHealth}),source);const n=norm(v(a.x-r.pos.x,.5,a.z-r.pos.z)),braced=this._knockbackScale(a);a.vx+=n.x*8*braced;a.vz+=n.z*8*braced;a.vy+=4*braced;}}for(const vehicle of this.vehicles){const d=Math.hypot(vehicle.position.x-r.pos.x,vehicle.position.z-r.pos.z);if(vehicle.health>0&&d<w.radius&&(d<w.radius*.6||this.visible(blast,vehicle.position))&&!this.vehicleFriendlyFire(vehicle,source))this.damageVehicle(vehicle,w.splash*affinityDamage*(1-d/w.radius),source);}this.emit('explosion',{pos:{...r.pos},weapon:r.weapon??1});}
  ...
  a.slow=Math.max(0,(a.slow||0)-dt);a.cooldown=Math.max(0,a.cooldown-dt);a.active=Math.max(0,a.active-dt);a.shotWait=Math.max(0,a.shotWait-dt);a.grenadeCooldown=Math.max(0,(a.grenadeCooldown||0)-dt);a.protection=Math.max(0,a.protection-dt);a.weaponSwitch=Math.max(0,(a.weaponSwitch||0)-dt);a.burstTimer=Math.max(0,(a.burstTimer||0)-dt);if(a.burstTimer<=0)a.burst=0;a.melee=Math.max(0,(a.melee||0)-dt);if(a.reloading){a.reloadTimer-=dt;if(a.reloadTimer<=0){const reloadWeapon=a.reloadWeapon,w=WEAPONS[reloadWeapon],cap=a.reloadCap??w?.cap,amount=a.reloadAmount??w?.ammo??0;a.ammo[reloadWeapon]=this.config.unlimitedAmmo?Infinity:Math.min(Number.isFinite(cap)?cap:Infinity,(a.ammo[reloadWeapon]||0)+amount);a.reloading=false;a.reloadTimer=0;a.reloadDuration=0;a.reloadWeapon=-1;a.reloadCap=undefined;a.reloadAmount=undefined;this.emit('reload',{actor:a.id,weapon:reloadWeapon,state:'end'});}}const wsel=this.weaponFor(a),recoil=wsel.recoil||{kick:0,recover:12},bloom=wsel.bloom,recover=recoil.recover??12;a.punchPitch=(a.punchPitch||0)+(a.punchVelPitch||0)*dt;a.punchYaw=(a.punchYaw||0)+(a.punchVelYaw||0)*dt;a.punchVelPitch=(a.punchVelPitch||0)+(-a.punchPitch*recover*recover-(a.punchVelPitch||0)*2*recover)*dt;a.punchVelYaw=(a.punchVelYaw||0)+(-a.punchYaw*recover*recover-(a.punchVelYaw||0)*2*recover)*dt;a.punchPitch=clamp(a.punchPitch,-.4,.4);a.punchYaw=clamp(a.punchYaw,-.4,.4);a.spread=Math.max(0,(a.spread||0)-(bloom?.recovery??.1)*dt);let expired=false;for(const id of Object.keys(a.powerups)){a.powerups[id]-=dt;if(a.powerups[id]<=0){delete a.powerups[id];expired=true;}}if(expired){this.refreshPowerups(a);if(!a.powerups.overshield)a.temporaryShield=0;}
   ...
   let controls=ext||(a.bot?this.botInput(a,dt):{});
     ...
     const firing=Boolean((ext&&ext.fire===true)||(a.bot&&a.bot.fired===true)||(a.burstLeft>0&&a.shotWait<=0&&a.ammo[a.weapon]>0));
