...
export function raceDisplay(snapshot,actorId=0){
 ...
 return {position:row?.position??'-',total:race?.standings?.length??8,lap:Math.min(laps,row?.lap??1),laps,checkpoint:row?row.nextGate+1:1,gates:race?.gates?.length??0,time:raceTime(row?.finishTime??race?.elapsed??0),item:row?.item?.toUpperCase()??'NO ITEM',countdown:race?.phase==='countdown'?String(Math.max(1,Math.ceil(race.countdown))):race?.phase==='finished'?'FINISHED':race?.elapsed<1?'GO!':'',effects:Object.entries(row?.effects??{}).filter(([,v])=>v>0).map(([key,v])=>`${key.toUpperCase()} ${v.toFixed(1)}s`).join(' / ')};
}
