...
export class SoftwareRenderer{
 ...
 render(scene,camera){
  ...
  const colors=new Map(),toSRGB=c=>c<.0031308?c*12.92:1.055*Math.pow(c,.41666)-.055;
  ...
  scene.traverseVisible(obj=>{
   ...
   for(let instanceIndex=0;instanceIndex<instances;instanceIndex++){
    ...
    for(let i=0;i<ix.length;i+=3){if(triangles>=this.triangleBudget)break;const a=vertices[ix[i]],b=vertices[ix[i+1]],c=vertices[ix[i+2]];if(!a||!b||!c||(a[2]>=-.1&&b[2]>=-.1&&c[2]>=-.1))continue;const poly=clip([a,b,c]);if(poly.length<3)continue;const p=poly.map(project),area=(p[1][0]-p[0][0])*(p[2][1]-p[0][1])-(p[1][1]-p[0][1])*(p[2][0]-p[0][0]),back=mat.side===T.BackSide;if(((mat.side===T.FrontSide||back)&&(back?area<=0:area>=0))||Math.abs(area)<this.minScreenArea)continue;
     ...
     let tr,tg,tb;if(cached.colors){const vc=cached.colors,i0=ix[i]*3,i1=ix[i+1]*3,i2=ix[i+2]*3;tr=(vc[i0]+vc[i1]+vc[i2])/3;tg=(vc[i0+1]+vc[i1+1]+vc[i2+1])/3;tb=(vc[i0+2]+vc[i1+2]+vc[i2+2])/3;}
     const depth=-(a[2]+b[2]+c[2])/3;draw.push({depth:obj.renderOrder===100?-100:depth,points:p,color:color(mat,light,mat.opacity,tr,tg,tb),line:mat.wireframe,closed:true});triangles++;}
   }
  });
